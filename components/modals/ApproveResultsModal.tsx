...
export const ApproveResultsModal: React.FC<ApproveResultsModalProps> = ({
  ...
  const handleApprove = async () => {
    ...
    try {
      let resolvedSpecifyId = String(specifyId || '').trim() || undefined;
      if (!resolvedSpecifyId) {
        const metaRes = await patientMetadataService.getById(labcode);
        const sid = metaRes.success ? String((metaRes.data as any)?.specifyId || '').trim() : '';
        resolvedSpecifyId = sid || undefined;
      }

      await approvePatientMetadataResultsOutput({ labcode, specifyId: resolvedSpecifyId });
