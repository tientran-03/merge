...
import { getOrderStatusBadge, ORDER_STATUS_OPTIONS } from "@/lib/constants/order-status";
...
export default function OrdersScreen() {
  ...
  const [statusTargetOrder, setStatusTargetOrder] = useState<OrderResponse | null>(null);

  const updateOrderStatusMutation = useMutation({
    mutationFn: async ({ orderId, status }: { orderId: string; status: string }) => {
      const response = await orderService.updateStatus(orderId, status);
      if (!response.success) {
        throw new Error(response.error || "Không thể cập nhật trạng thái");
      }
      return response;
    },
    onSuccess: () => {
  ...
  const openOrderStatusModal = (order: OrderResponse) => {
    setStatusTargetOrder(order);
    setShowOrderStatusModal(true);
  };

  const handleSelectOrderStatus = (value: string) => {
    if (!statusTargetOrder?.orderId) return;
    if ((statusTargetOrder.orderStatus || "").toLowerCase() === value.toLowerCase()) {
      setShowOrderStatusModal(false);
      setStatusTargetOrder(null);
      return;
    }
    updateOrderStatusMutation.mutate({ orderId: statusTargetOrder.orderId, status: value });
  };
  ...
  const dayKeys = useMemo(
    () =>
      Object.keys(groupedOrders).sort((a, b) => {
        const maxA = Math.max(
          ...groupedOrders[a].map((o: any) => (o.createdAt instanceof Date ? o.createdAt.getTime() : 0))
        );
        const maxB = Math.max(
          ...groupedOrders[b].map((o: any) => (o.createdAt instanceof Date ? o.createdAt.getTime() : 0))
        );
        return maxB - maxA;
      }),
    [groupedOrders]
          ...
          dayKeys.map((date) => (
            <View key={date} className="mb-6">
              ...
              {groupedOrders[date].map((order: any) => {
                ...
                const full: OrderResponse = order.fullOrder;
                const orderStatusBadge = getOrderStatusBadge(full?.orderStatus || "");
      ...
      >
        <View className="flex-1 bg-black/50 justify-center items-center p-5">
          <View className="bg-white rounded-2xl p-6 w-full max-w-[400px]">
            <Text className="text-lg font-extrabold text-slate-900 mb-1">Cập nhật trạng thái</Text>
            <Text className="text-xs text-slate-500 mb-1" numberOfLines={2}>
              Đơn: {statusTargetOrder?.orderName || statusTargetOrder?.orderId || "—"}
            </Text>
            <Text className="text-[10px] text-slate-400 mb-2">
              Mã: {statusTargetOrder?.orderId || "—"}
            </Text>
            {updateOrderStatusMutation.isPending && (
              <View className="mb-3 items-center">
                <ActivityIndicator size="small" color="#0284C7" />
                <Text className="text-xs text-slate-500 mt-1 font-semibold">Đang cập nhật…</Text>
              </View>
            )}
            ...
            <ScrollView className="max-h-72 mb-2" showsVerticalScrollIndicator={false}>
              {ORDER_STATUS_OPTIONS.map((opt) => {
                const isCurrent =
                  (statusTargetOrder?.orderStatus || "").toLowerCase() === opt.value.toLowerCase();
                return (
                  <TouchableOpacity
                    key={opt.value}
                    onPress={() => handleSelectOrderStatus(opt.value)}
                    className={`px-4 py-3 rounded-xl border mb-2 ${
                      isCurrent ? "bg-sky-50 border-sky-500" : "bg-white border-slate-200"
                    }`}
                    activeOpacity={0.85}
                    disabled={isCurrent || updateOrderStatusMutation.isPending}
                  >
                    <View className="flex-row items-center justify-between">
                      <Text
                      ...
                      {isCurrent ? <Text className="text-sky-600 text-sm font-extrabold">✓</Text> : null}
                    </View>
                  </TouchableOpacity>
                );
              })}
            ...
            <TouchableOpacity
              ...
              className="py-3 rounded-xl border border-slate-200 items-center"
              activeOpacity={0.85}
              disabled={updateOrderStatusMutation.isPending}
            >
