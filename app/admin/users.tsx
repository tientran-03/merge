...
function FilterPill({
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      className={`px-4 py-2 rounded-full border ${active ? "bg-sky-600 border-sky-600" : "bg-white border-slate-200"
        }`}
      activeOpacity={0.7}
    >
      <Text
...
export default function AdminUsersScreen() {
  ...
  const blockMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setBlockReason("");
      presentFeedbackSuccess({ title: "Thành công", message: "Đã khóa tài khoản người dùng" });
    },
    onError: (error: any) => {
      presentFeedbackError({ title: "Lỗi", message: error.message || "Không thể khóa tài khoản" });
    },
  });
  ...
  const unblockMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      presentFeedbackSuccess({ title: "Thành công", message: "Đã mở khóa tài khoản người dùng" });
    },
    onError: (error: any) => {
      console.error("❌ Unblock error:", error);
      presentFeedbackError({
        title: "Lỗi",
        message: error.message || "Không thể mở khóa tài khoản. Vui lòng thử lại.",
      });
    },
  });
  ...
  const handleBlockUser = () => {
    ...
    if (!blockReason.trim()) {
      presentFeedbackError({ title: "Lỗi", message: "Vui lòng nhập lý do khóa tài khoản" });
      return;
    }
    ...
    >
          ...
          <View className="p-4">
            {filteredUsers.map((userItem, index) => {
              ...
              return (
                <View
                  key={userItem.userId}
                  className={`bg-white rounded-2xl p-4 mb-3 border border-sky-100 ${index === 0 ? "" : ""
                    }`}
                >
                  {/* User header */}
      ...
      >
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-white rounded-t-3xl p-6 max-h-[85%]" style={{ paddingBottom: sheetBottomInset }}>
                  ...
                  <View className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
                    ...
                    <View className="gap-2">
                      <View className="flex-row items-center justify-between">
                        ...
                        <View
                          className={`px-2 py-1 rounded ${selectedUser.enabled
                              ? "bg-emerald-50 border border-emerald-200"
                              : "bg-orange-50 border border-orange-200"
                            }`}
                        >
                          <Text
                            className={`text-[10px] font-bold ${selectedUser.enabled ? "text-emerald-700" : "text-orange-700"
                              }`}
                          >
                      ...
                      <View className="flex-row items-center justify-between">
                        ...
                        <View
                          className={`px-2 py-1 rounded ${selectedUser.isActive
                              ? "bg-emerald-50 border border-emerald-200"
                              : "bg-red-50 border border-red-200"
                            }`}
                        >
                          <Text
                            className={`text-[10px] font-bold ${selectedUser.isActive ? "text-emerald-700" : "text-red-700"
                              }`}
                          >
