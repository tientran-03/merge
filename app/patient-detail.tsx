...
/** Nhãn trạng thái phiếu (khớp prescription-slips / admin specifies). */
function labelSpecifyStatusVi(status?: string | null): string {
  if (!status) return "—";
...
export default function PatientDetailScreen() {
  ...
  const code = getPatientMbnDisplay(patient);
                      ...
                      {s.specifyStatus && (
                        <View className="px-2 py-1 rounded-full bg-slate-50 border border-slate-200">
                          <Text className="text-[11px] font-extrabold text-slate-600">
                            {labelSpecifyStatusVi(s.specifyStatus)}
                          </Text>
                        </View>
                      )}
                    </View>
