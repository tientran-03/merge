...
import { ROLE_ADMIN, ROLE_CUSTOMER, ROLE_LAB_TECHNICIAN } from '@/constants/roles';
...
export default function LabTechnicianHomeScreen() {
  ...
  const menuItems: MenuItem[] = useMemo(
    () => [
      ...
      {
        id: 'patient-test-results',
        title: 'Kết quả\nxét nghiệm',
        icon: <Image source={require('@/assets/staff/i08_quan_ly_phieu_xn.png')} className="w-16 h-16" resizeMode="contain" />,
        route: '/staff/patient-test-results',
      },
      {
        id: 'patients',
        title: 'Danh sách\nbệnh nhân',
      ...
      },
      {
        id: 'genome-tests',
        title: 'Danh sách\nxét nghiệm',
        icon: <Image source={require('@/assets/staff/i04_quan_ly_dich_vu.png')} className="w-16 h-16" resizeMode="contain" />,
        route: '/staff/genome-tests',
      },
      {
        id: 'profile',
        title: 'Thông tin\n& bảo mật',
        icon: <Image source={require('@/assets/staff/i07_thong_tin_nguoi_dung.png')} className="w-16 h-16" resizeMode="contain" />,
        route: '/lab-technician/profile',
      },
      {
        id: 'logout',
        title: 'Đăng xuất',
  ...
  if (isLoading || !user || user.role !== ROLE_LAB_TECHNICIAN) {
    return (
      ...
      >
        <View className="w-64 h-16 flex-row items-center justify-between">
          ...
          <View className="bg-amber-600 px-3 py-1 rounded-full">
            <Text className="text-white text-xs font-bold">KỸ THUẬT VIÊN</Text>
          </View>
        </View>
      </ImageBackground>
      <ScrollView
