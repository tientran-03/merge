...
export const logService = {
  querySystemLogs: async (params: SystemLogsQueryParams) => {
    // Backend expects JSON body (not querystring-only), otherwise returns VALIDATION_012.
    // Keep keys stable with web: keyword/level/limit.
    const body: Record<string, unknown> = {};
    if (params.keyword) body.keyword = params.keyword;
    if (params.level) body.level = params.level;
    if (params.limit !== undefined) body.limit = params.limit;
    return apiClient.post<SystemLogsResponse>(API_ENDPOINTS.SYSTEM_LOGS_QUERY, body);
  },
